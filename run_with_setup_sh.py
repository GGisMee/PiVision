...
def run_python_script(script_path, flags):
    ...
    try:
        ...
        subprocess.run(['python', script_path] + flags.split(), check=True)
        print(f"Successfully ran {script_path}")
    except subprocess.CalledProcessError as e:
        print(f"Error running {script_path}: {e}")
        exit(1)
...
if __name__ == "__main__":
    ...
    detection_flag = detection_flags[0]
    # Set the paths to the scripts
    setup_script = './setup_env.sh'  # Adjust the path if needed
    detection_script = './basic_pipelines/detection.py'  # Adjust the path if needed
